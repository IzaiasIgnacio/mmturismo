...
class ici extends TCPDF {
	...
	function exibir($dados,$x=5) {
		...
		$this->Cell(101.6,0.7,'',0,2,'L',false,'',0,true);
		$this->Cell(101.6,6,mb_convert_case(utf8_encode($dados['cliente']),MB_CASE_TITLE,'UTF-8'),0,2,'L',false,'',0,true);
		$complemento = ($dados['complemento'] == '') ? '' : ' - '.$dados['complemento'];
		$this->Cell(101.6,6,mb_convert_case(utf8_encode($dados['endereco']),MB_CASE_TITLE,'UTF-8').', '.$dados['numero'].mb_convert_case(utf8_encode($complemento),MB_CASE_TITLE,'UTF-8'),0,2,'L',false,'',1,true);
		$this->Cell(101.6,6,'Bairro: '.mb_convert_case(utf8_encode($dados['bairro']),MB_CASE_TITLE,'UTF-8'),0,2,'L',false,'',0,true);
		$this->Cell(101.6,6,mb_convert_case(utf8_encode($dados['cidade']),MB_CASE_TITLE,'UTF-8').' - '.$dados['sigla'].' CEP: '.$dados['cep'],0,2,'L',false,'',1,true);
		$this->Cell(101.6,1.4,'',0,2,'L',false,'',0,true);
	}
}
