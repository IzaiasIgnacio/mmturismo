...
if (count($lista_transportes) > 0) {
	...
	foreach ($lista_transportes as $l) {
		...
		for ($i=1;$i<=$l['quantidade'];$i++) {
			...
			$pdf->Cell(0, 0, "Embarque ".$_POST['viagem']." ".$_POST['data']." (".$dia_semana.") ".$l['empresa']." - ônibus ".$i, 1, 1, 'C', 0, '', 1);
			$transportes[$t] = "Embarque ".$_POST['viagem']." ".$_POST['data']." (".$dia_semana.") ".$l['empresa']." - ônibus ".$i;
			$pdf->Ln(1);
			$lista_pontos = $viagem -> passageiros_pontos($l['id'],$i);
			$c = 0;
			//pontos de embarque de cada onibus
			if (count($lista_pontos) > 0) {
				$pdf->Cell(20, 0, 'Horário', 0, 0, 'L', 0, '', 1);
	...
	$pdf->Cell(0, 0, 'Nenhum ônibus na viagem', 0, 1, 'C', 0, '', 1);
}
