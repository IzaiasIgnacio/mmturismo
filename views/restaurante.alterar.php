<?php
require_once('controles/restaurante.php');
controle_restaurante('alterar');
?>
<script type="text/javascript">
	...
	$(function() {
		...
		$("#restaurante").tooltip("option","content","Informe o nome do restaurante");
		$("#estado").tooltip("option","content","Escolha o estado do restaurante");
		$("#cidade").tooltip("option","content","Escolha a cidade do restaurante");
		...
		//autocompletar busca de restaurante
		$("#buscar_restaurante").autocomplete({
			source: function(request, response){
				//buscar restaurante
				$.post("controles/restaurante.jquery.php", {acao:'buscar_restaurante',nome:request.term},
				function(resposta){
					response($.map(resposta,
					function(item) {
						return {
							//valor para exibir na selecao
							label: item.restaurante,
							//valor para exibir na tabela e campo de busca
							nome: item.restaurante,
							//id do restaurante
							value: item.id
						};
					}));
			...
			select: function(event, ui) {
				...
				if (ui.item.value != 0) {
					carregar_restaurante(ui.item.value);
				}
			}
		});
		...
		<?php if ($_POST) { ?>
		carregar_restaurante(<?php echo $_POST['id_restaurante']; ?>);
		<?php } ?>
	});
	...
	function carregar_restaurante(id_restaurante) {
		//buscar dados do restaurante escolhido
		$.post('controles/restaurante.jquery.php',{acao:'buscar_dados',restaurante:id_restaurante},
		function(resposta) {
			//telefones do restaurante
			telefones = new Array();
			//excluir telefones
			...
			$("#buscar_restaurante").val(dados['restaurante']);
			$("#id_restaurante").val(dados['id']);
			$("#restaurante").val(dados['restaurante']);
			$("#cnpj").val(dados['cnpj']);
			...
			//telefones
			$.post('controles/restaurante.jquery.php',{acao:'exibir_telefones_consulta',id_restaurante:dados['id']},
			function(resposta) {
			...
			//bancos
			$.post('controles/restaurante.jquery.php',{acao:'exibir_bancos_consulta',id_restaurante:dados['id']},
			function(resposta) {
	...
	function exibir_historico(inicio,pagina) {
		$.post('controles/restaurante.jquery.php',{acao:'buscar_historico',restaurante:$("#id_restaurante").val(),inicio:inicio,
...
<form id='form' method='post'>
	...
	<div class='titulo_tela'>Consultar / Alterar Restaurante</div>
	...
	<div class='conteudo'>
		<fieldset id='fieldset_consulta'>
			<span class='titulo_dados'>Buscar Restaurante</span>
			<label class='label_campo esquerda'>Nome: </label>
			<input type='text' name='buscar_restaurante' id='buscar_restaurante' size='81' maxlength='70'>
		</fieldset>
	</div>
	<div class='conteudo' id='dados_restaurante' style='display:none'>
		<fieldset>
			<span class='titulo_dados'>Dados do restaurante</span>
			<label class='label_campo esquerda'>Nome: </label>
			<input type='text' name='restaurante' id='restaurante' size='80' maxlength='70'>
