...
<script type="text/javascript">
	...
	var html_hoteis = "<option value=''>Selecione</option>";
	$(function() {
		//campos obrigatorios
		$("#form").validate({
			rules: {
		...
		$("#formulario_destino").dialog({
			buttons:{
				...
				Adicionar: function() {
					...
					if (add == 1) {
						//adicionar ao array
						destinos[destinos.length] = new Array(cidade);
			...
			buttons:{
				...
				Adicionar: function() {
					...
					if (add == 1) {
						...
						restaurantes[posicao] = new Array(restaurante);
			...
			buttons:{
				...
				Adicionar: function() {
					...
					if (add == 1) {
						...
						hoteis[posicao] = new Array(hotel);
						//criar linha na tabela de restaurantes
						var html;
						html = "<tr class='add_linha'>";
		...
		$("#buscar_cliente").livequery(function() {
			$(this).autocomplete({
				...
				select: function(event, ui) {
					...
					//cliente encontrado
					if (ui.item.value != 0) {
						//verificar se o cliente j� est� na viagem
						if(!verificar_cliente(ui.item.value)) {
							//adicionar cliente ao array
							var posicao = clientes.length;
							clientes[clientes.length] = new Array(ui.item.value,ui.item.nome);
							...
							caixa_mensagem('Aviso','O cliente j� est� inclu�do nessa viagem');
						}
					}
				}
			});
			...
			switch (tabela) {
				...
				case 'transportes':
					...
					$(sinais_transportes).each(function(i) {
						if (sinais_transportes[i][2] == posicao[1]) {
				...
				case 'restaurantes':
					...
					$(sinais_restaurantes).each(function(i) {
						if (sinais_restaurantes[i][2] == posicao[1]) {
				...
				case 'hoteis':
					...
					$(sinais_hoteis).each(function(i) {
						if (sinais_hoteis[i][2] == posicao[1]) {
		...
		$("#transportes, #restaurantes, #hoteis").on('click','.btn_remover_sinal',function(event) {
			...
			switch (tabela[1]) {
				case 'transportes':
					delete(sinais_transportes[tabela[2]]);
				break;
				case 'restaurantes':
	...
	//verificar se um cliente j� est� na viagem
